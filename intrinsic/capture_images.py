...
def draw_debug_info(image, corners_found, capture_count, max_captures, blur_score=0, sharpness_score=0, quality_ok=True):
    ...
    if ENABLE_BLUR_DETECTION and corners_found:
        ...
        quality_color = (0, 255, 0) if quality_ok else (
...
def main():
    ...
    detection_stats = {"total_frames": 0,
                       "detected_frames": 0, "quality_rejected": 0}
    ...
    try:
        while capture_count < MAX_CAPTURES:
            ...
            if corners_found:
                ...
                cv2.drawChessboardCorners(
                    frame, CHESSBOARD_SIZE, corners, corners_found)
                ...
                if (current_time - last_capture_time > CAPTURE_INTERVAL and
                    ...
                    print(
                        f"Captured image {capture_count}/{MAX_CAPTURES}: {filename}{quality_info}")

                    if capture_count >= MIN_CAPTURES:
                        print(
                            f"Minimum captures ({MIN_CAPTURES}) reached. Continue for better accuracy or press 'q' to stop.")
            ...
            if key == ord('q'):
                ...
                if ENABLE_BLUR_DETECTION:
                    ...
                    if not quality_ok:
                        ...
                        print(
                            "Try better lighting, steadier hands, or move closer/farther")
                        continue
                ...
                quality_info = f" (Quality: {quality_reason})" if ENABLE_BLUR_DETECTION else ""
                print(
                    f"Manually captured image {capture_count}/{MAX_CAPTURES}: {filename}{quality_info}")
        ...
        print(
            f"Detection rate: {detection_stats['detected_frames']}/{detection_stats['total_frames']} frames ({detection_stats['detected_frames']/max(detection_stats['total_frames'], 1)*100:.1f}%)")

        if ENABLE_BLUR_DETECTION:
            print(
                f"Quality rejections: {detection_stats['quality_rejected']} frames")
            quality_rate = (detection_stats['detected_frames'] - detection_stats['quality_rejected']) / max(
                detection_stats['detected_frames'], 1) * 100
        ...
        if capture_count >= MIN_CAPTURES:
            ...
            print(
                f"⚠ Warning: Only {capture_count} images captured. Recommended minimum: {MIN_CAPTURES}")
